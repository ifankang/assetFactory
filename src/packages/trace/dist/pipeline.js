...
import { rgbMapGaussian, brightnessThreshold, grayMapCanny, quantizeBand, } from '@vectrace/preprocess';
import { rgbMapQuantize } from '@vectrace/quantize';
import { grayMapToPath, ensurePotraceInit } from './potrace.js';
function toPotraceParams(params) {
    return {
...
function filterIndexed(rgbMap, params) {
    let map = rgbMap;
...
async function traceBrightnessMulti(rgbMap, params) {
    const low = 0.2;
    const high = 0.9;
...
async function traceQuant(rgbMap, params) {
...
export async function trace(rgbMap, params) {
