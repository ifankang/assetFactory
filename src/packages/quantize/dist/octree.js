import { Ref } from './pool.js';
function shiftRGB(rgb, s) {
    return { r: rgb.r >> s, g: rgb.g >> s, b: rgb.b >> s };
}
function eqRGB(rgb1, rgb2) {
    return rgb1.r === rgb2.r && rgb1.g === rgb2.g && rgb1.b === rgb2.b;
}
export function childIndex(rgb) {
    return ((rgb.r & 1) << 2) | ((rgb.g & 1) << 1) | (rgb.b & 1);
}
export function distRGB(rgb1, rgb2) {
    return (rgb1.r - rgb2.r) ** 2 + (rgb1.g - rgb2.g) ** 2 + (rgb1.b - rgb2.b) ** 2;
}
export function findRGB(palette, ncolor, rgb) {
    let index = -1;
    let bestDist = 0;
...
export function ocnodeNew(pool) {
    ...
    node.children = [];
    for (let i = 0; i < 8; i++) {
        node.children.push(new Ref(null));
    }
    node.mi = 0;
    return node;
}
export function ocnodeFree(pool, node) {
    pool.drop(node);
}
export function ocnodeLeaf(pool, ref, rgb) {
...
export function octreeDelete(pool, node) {
    if (!node)
        return;
    for (let i = 0; i < 8; i++) {
...
export function octreeMerge(pool, parent, ref, node1, node2) {
    if (!node1 && !node2)
        return 0;
    if (parent && !ref.value)
        parent.nchild++;
    ...
    if (dwitdth > 0 && eqRGB(node1.rgb, shiftRGB(node2.rgb, dwitdth))) {
        ...
        node1.mi = 0;
        if (node1.children[i].value)
            node1.nleaf -= node1.children[i].value.nleaf;
        ...
        node2.mi = 0;
        if (node2.children[i].value)
            node2.nleaf -= node2.children[i].value.nleaf;
            ...
            if (node1.nchild === 0 && node2.nchild === 0) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (node1.children[i].value || node2.children[i].value) {
                        newnode.nleaf += octreeMerge(pool, newnode, newnode.children[i], node1.children[i].value, node2.children[i].value);
                    }
                }
            }
...
export function ocnodeMi(node) {
    node.mi = node.parent ? node.weight << (2 * node.parent.width) : 0;
}
export function ocnodeStrip(pool, ref, count, lvl) {
    ...
    if (node.nchild === 0) {
        ...
        if (node.mi && node.mi > lvl)
            return;
        node.nchild = 0;
        node.nleaf = 0;
        node.mi = 0;
        let lonelychild = null;
        for (let i = 0; i < 8; i++) {
...
export function octreePrune(pool, ref, ncolor) {
    if (!ref.value || ncolor <= 0)
        return;
...
export function octreeBuildArea(pool, rgbmap, ref, x1, y1, x2, y2, ncolor) {
    const dx = x2 - x1;
    const dy = y2 - y1;
    ...
    if (dx === 1 && dy === 1) {
        ...
        const ref1 = new Ref(null);
        const ref2 = new Ref(null);
