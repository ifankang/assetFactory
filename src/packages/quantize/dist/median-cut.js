function colorRange(box) {
    ...
    if (rr >= rg && rr >= rb)
        return { dim: 'r', range: rr };
    if (rg >= rr && rg >= rb)
        return { dim: 'g', range: rg };
...
export function medianCutQuantize(rgbmap, ncolor) {
    ...
    while (boxes.length < ncolor && boxes.length < pixels.length) {
        ...
        if (bestIdx === -1 || boxes[bestIdx].pixels.length < 2)
            break;
...
//# sourceMappingURL=median-cut.js.map