...
const ARGS_PER_COMMAND: Record<string, number> = {
  m: 2, l: 2, h: 1, v: 1,
  c: 6, s: 4, q: 4, t: 2, a: 7,
}

function add(
  mnx: number, mny: number, mxx: number, mxy: number,
  x: number, y: number,
 ...
 */
function tokenize(d: string): { cmd: string; rel: boolean; args: number[] }[] {
  COMMAND_RE.lastIndex = 0
  const out: { cmd: string; rel: boolean; args: number[] }[] = []
  let lastIdx = 0
  ...
  while ((m = COMMAND_RE.exec(d)) !== null) {
    ...
    if (lastIdx > 0 && out.length > 0) {
      ...
      if (nums) out[out.length - 1].args.push(...nums.map(Number))
    }
...
export function parsePathBounds(d: string): Bounds {
