...
import type { TraceResult } from '@vectrace/core'
import { parsePathBounds } from './pathBounds.js'

function unionBounds(all: { minX: number; minY: number; maxX: number; maxY: number }[]) {
...
export function traceResultToSvg(
  results: TraceResult,
  width: number,
  height: number,
): string {
